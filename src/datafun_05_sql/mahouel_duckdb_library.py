"""
mahouel_duckdb_library.py - Project script (custom).

Author: Abdelhafidh Mahouel
Date: 2026-02

Purpose:
- Read csv files into a DuckDB database (Library domain).
...
OBS:
  This is the custom file for the Library domain.
"""
...
def main() -> None:
    """Run the pipeline."""
    log_header(LOG, "P05 Pipeline Custom (DuckDB - Library)")
    ...
    try:
        ...
        # ----------------------------------------------------
        run_sql_script(con, SQL_DIR / "mahouel_library_bootstrap.sql")

        # ----------------------------------------------------
        # STEP 1: CLEAN
        # ----------------------------------------------------
        run_sql_script(con, SQL_DIR / "mahouel_library_clean.sql")

        # ----------------------------------------------------
        # STEP 3: RUN KPI QUERY
        # ----------------------------------------------------
        run_sql_query(con, SQL_DIR / "mahouel_library_query_kpi_checkouts.sql")
